...
class BuildCppExtension(build_ext):
    ...
    def run(self):
        ...
        if not makefile_path.exists() or not helpers_cpp_path.exists():
            ...
            print("You can manually compile later by running: make -C utils/")
            return
        ...
        try:
            ...
            print("  Then manually compile: cd utils/ && make")
...
setup(
    ...
    python_requires=">=3.8",
    install_requires=parse_requirements() + ["pybind11>=2.10.0"],  # Required for C++ extension compilation
    extras_require={
        "dev": [
            "pytest>=7.0.0",
            "black>=23.0.0",
    ...
    # Users can also manually compile by running: make -C utils/
